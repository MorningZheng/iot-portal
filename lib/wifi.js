...
var $this=module.exports={
    ...
    mac:{
        ...
        reject:function (mac,deadline) {
            ...
            if(mac){
                ...
                if(files.file.readTxtSync(file).indexOf(mac)===-1){
        ...
        accept:function (mac) {
            ...
            if(mac) {
                ...
                var file=files.hostapd.mac_file_deny;
                var data=files.file.readTxtSync(file);
    ...
    release:function () {
        var file=files.hostapd.mac_file_deny;
        var data=files.file.readTxtSync(file).split('\n');
    ...
    password:{
        reset:function (txt) {
            txt=txt||files.file.readTxtSync(files.wifi.password);
            $this.password.data=txt.split(/[\r\n\,\;]+/).filter(function (p) {
                return p.length>1;
            });
        },
        data:undefined,
    ...
    edit:{
        ...
        address:function (value) {
            ...
            if(value){
                ...
                value=files.file.readTxtSync(files.dnsmasq.hosts).split('\n').map(function (v) {
