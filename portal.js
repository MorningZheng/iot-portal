(function () {
    ...
    /**
     * ip地址密码验证
     * @param from 从拿个渠道来，会分q(二维码)和p(portal身份认证)
     * @param ip 要验证的ip地址
     * @param password 密码
    ...
    const checkin=(function () {
        ...
        return function (from,ip,password) {
            if(ip==='127.0.0.1')return true;
            else if(wifi.checkin(password)){
                bridge.dispatch(files.events.wifi.checkin,{from:from,mac:wifi.ip.pass(ip),password:password});
                // wifi.ip.pass(ip);
                return true;
            }else{
        ...
        return {
            ...
            request:function (req,res) {
                ...
                if(req.get.pathname==='/door.php'){
                    if (req.method === 'POST'){
                        ...
                        req.on('end', function () {
                            try{
                                ...
                                if(data instanceof Array)data=JSON.stringify(data[0]==='checkin'?checkin('p',ip(req.socket),data[1][0]):require('./lib/rpc')(data[0],data[1]));
                    ...
                    page.welcome(req,res,checkin('q',ip(req.socket), req.url.substr(2)));
                    return;
                };
